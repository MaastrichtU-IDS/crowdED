# We need a function with fixed incomes and return:
# df_tw, p_consensus, m_nconsensus
...
label_worker = ['good_worker' if workers[i]
                in good_workers else 'poor_worker' for i in range(total_workers)]
...
for i in label_worker:
    if i == 'good_worker':
        # a random number form cut to 1
        probs_workers.append(np.random.choice(
            (np.arange(cut_workers, 1, 0.01)), 1))
    elif i == 'poor_worker':
        # a random number form chance to 1
        probs_workers.append(np.random.choice(
            (np.arange(0.5, cut_workers, 0.01)), 1))
    else:
...
p_train_tasks = float(
    input("Percentage of Tasks to Train (decimal): "))  # .4 #percentage
...
list_tasks1 = [[training_tasks[i]] *
               workers_per_task for i in range(tasks_to_train)]
# flat and clean list
...
tasks_workers1 = [np.random.choice(
    workers, workers_per_task, replace=False) for task in training_tasks]
# flat and clean list
...
tasks_mean1 = df_tw1.groupby('task_id').mean(
).sort_values('performance', ascending=False)
first_tasks = tasks_mean1[tasks_mean1['performance']
                          >= cutoff_task]['performance']

# - Trained Workers
workers_mean1 = df_tw1.groupby('worker_id').mean(
).sort_values('performance', ascending=False)
df_workers = pd.merge(workers_mean1.reset_index(), df_workers.drop(
    'prob_worker', 1), on='worker_id', how='left')
...
for i in tasks_redo_unique:
    ...
    each = [i for i in tasks_redo[tasks_redo['task_id']
                                  == str(i)]['worker_id']]
    ...
    if len(rest_workers) > workers_per_task:
        tasks_workers2.append(np.random.choice(
            rest_workers, workers_per_task, replace=False))
    else:
...
list_tasks2 = [[tasks_redo_unique[i]] * workers_per_task for i in range(
    len(tasks_redo_unique))]  # replicate the tasks to train
# flat and clean list
...
# - **Tasks Un-done**
tasks_undone_unique = df_tasks[~df_tasks['task_id'].isin(
...
for i in tasks_undone_unique:
    tasks_workers3.append(np.random.choice(
        best_workers, workers_per_task, replace=False))

# flat and clean list
tasks_workers3 = [worker for sublist in tasks_workers3 for worker in sublist]

list_tasks3 = [[tasks_undone_unique[i]] * workers_per_task for i in range(
...
tasks_mean = df_tw.groupby('task_id').mean().sort_values(
    'performance', ascending=False)
final_tasks = tasks_mean[tasks_mean['performance']
                         >= cutoff_task]['performance']
